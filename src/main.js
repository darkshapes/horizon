...
class MultiTouchSlider {
    ...
    handleTouchStart(e) {
        ...
        e.changedTouches.forEach(touch => {
            ...
            this.activeTouches.set(touch.identifier, { key, startY: touch.clientY, startRaw: currentRaw, savedValue: currentRaw });
        });
    }
    ...
    handleTouchEnd(e) {
        ...
        e.changedTouches.forEach(touch => {
            ...
            if (channel) {
                ...
                if (newValue !== touchData.savedValue && this.saveHistoryEntry) this.saveHistoryEntry(channel);
    ...
    handlePointerDown(e) {
        ...
        const currentRaw = channel.value.getRawPercentage();
        this.activeTouches.set(e.pointerId, { key, startY: e.clientY, startRaw: currentRaw, savedValue: currentRaw, element: channel.element });
    }
    ...
    updateChannelDisplay(channelObj) {
        ...
        if (this.updateUndoRedoIndicator) {
            const key = Array.from(this.channels.entries()).find(([, c]) => c === channelObj)?.[0];
